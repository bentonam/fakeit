...
```bash
data-generator -d output/ -m users.yaml,products.yaml,orders.yaml
```
...
[~/examples/contacts]$ data-generator -d data/
...
[~/examples/ecommerce]$ data-generator -a export.zip
...
```bash
[~]$ data-generator -m ~/examples/music -i ~/examples/music/input -d ~/examples/music/output -a music.zip
...
```bash
[~]$ data-generator -o csv -m ~/examples/flat -d ~/csv
...
```bash
[~]$ data-generator -o csv -m ~/examples/flat -d ~/csv -a export.zip
[~]$ ls ~/csv | awk '{print "\011",$NF}'
	 export.zip
```
...
```bash
[~]$ data-generator -d couchbase -m ~/examples/music
```
...
```bash
[~]$ data-generator -d couchbase -m ~/examples/music -s 192.168.1.101 -b music -p secret
```
...
```bash
[~]$ data-generator -d sync-gateway -m ~/examples/contacts -s http://localhost:4984 -b contacts
```
...
```bash
[~]$ data-generator -d sync-gateway -m ~/examples/contacts -s http://localhost:4984 -b contacts -g http://localhost:4985 -u jdoe -p supersecret
```