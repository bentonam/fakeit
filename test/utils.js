...
const { reduce } = require('async-array-methods');
const fs = require('fs-extra-promisify');
const yaml = require('yamljs');
const json = require('jsondiffpatch');
...
module.exports.models = function (settings) {
  ...
  function models(cb, match, todo, run) {
    const options = to.arguments.apply(null, [
    ...
    return function (test) {
      ...
      settings.modules.forEach((model) => {
          ...
          test(model, function(t) { // eslint-disable-line
            ...
            return result
              .then((actual) => {
                ...
                const omitted = _.difference(to.keys(actual), schema_keys)
                  .filter((key) => ['__key', '__name'].indexOf(key) < 0);
                ...
                function validate(err) {
                  if (err) {
                    ...
                    if (segments) {
                      segments = segments.map((segment) => segment.replace(/child\s+|"/g, ''));
                      const item_path = segments.join('.');
                      const item = _.get(picked, item_path);
                      if (item) {
  ...
  models.getContents = function getContents() {
    return reduce(models.files, (prev, next) => fs.readFile(p(settings.root, next))
      .then((contents) => {
  ...
  models.todo = function () {
    for (var schema in models.schemas_todo) { // eslint-disable-line
      for (let i = 0; i < models.schemas_todo[schema].length; i++) {
